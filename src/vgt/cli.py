...
from .track_jobs import TRACK_JOB_BACKENDS, run_track_job
...
def _parser() -> argparse.ArgumentParser:
    ...
    track_parser = transcription_sub.add_parser(
        "track", help="On-demand single-track transcription jobs (docs/on-demand-track-transcription-plan.md)."
    )
    track_sub = track_parser.add_subparsers(dest="track_command", required=True)
    track_run = track_sub.add_parser(
        "run",
        help="Run one on-demand single-track transcription job to completion. Spawned detached by a "
        "ReaScript trigger action; not ordinarily run interactively.",
    )
    ...
    track_run.add_argument(
        ...
        help="GM program (0-127) to force every decoded note onto (MT3 only; recorded for every "
        "backend as track-label metadata).",
    )
    track_run.add_argument(
        "--backend", choices=TRACK_JOB_BACKENDS, default="mt3",
        help="Transcription backend to run this track through (default: mt3).",
    )
    ...
    if args.transcription_command == "track":
        if args.track_command == "run":
            ...
            status = run_track_job(
                ...
                backend=args.backend, label=args.label, progress=report,
            )
