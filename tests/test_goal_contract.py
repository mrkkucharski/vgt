...
def test_reascript_uses_beats_track_without_writing_markers_when_bar_phase_is_unknown(tmp_path: Path) -> None:
    project = _copy_project(tmp_path)
...
def test_reascript_leaves_a_non_default_map_untouched_when_bar_phase_is_unknown(tmp_path: Path) -> None:
    """A deliberate non-default map (e.g. Hotel California Cover's 150 BPM,
    #273/#274) remains untouched while the analyzed grid is shown on Beats."""
    project = _copy_project(tmp_path)
    project.with_suffix(".vgt").write_text(json.dumps({
        "config": {"reference_track_guid": REFERENCE_GUID},
        "analysis": {"tempo": {"value": {
...
def test_reascript_uses_beats_track_without_writing_markers_for_detected_downbeats(tmp_path: Path) -> None:
    project = _copy_project(tmp_path)
    project.with_suffix(".vgt").write_text(json.dumps({
        "config": {"reference_track_guid": REFERENCE_GUID},
        "analysis": {"tempo": {"value": {
    ...
    assert "[vgt] Beats" in names
    assert tempo_writes == "0"
    config = read_sidecar(project)["config"]
    assert config["tempo_map_applied"] is False
    assert config["tempo_map_fingerprint"] == ""
    assert config["tempo_data_fingerprint"] == ""
