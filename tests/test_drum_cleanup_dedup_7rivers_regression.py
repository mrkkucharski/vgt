...
def _ground_truth() -> dict[str, list[float]]:
    notes = json.loads((FIXTURES / "corrected_ground_truth.json").read_text())
    return instrument_onsets(notes)
...
def test_dedup_moves_7rivers_note_count_toward_truth_without_losing_raw_recall() -> None:
    ...
    before = score_onsets(truth, instrument_onsets(raw))
    after = score_onsets(truth, candidate)
    before_count = sum(len(times) for times in instrument_onsets(raw).values())
