...
def _require_disposable_output(path: Path, parser: argparse.ArgumentParser) -> None:
    parts = {part.lower() for part in path.resolve().parts}
    if {".vgt", "transcription"} & parts:
        parser.error("smoke output must be a disposable evaluation directory, not a vgt production artifact or sidecar path")
...
def main(argv: list[str] | None = None) -> int:
    ...
    _require_disposable_output(args.output, parser)
